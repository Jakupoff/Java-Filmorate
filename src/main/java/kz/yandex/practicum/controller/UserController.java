...
import kz.yandex.practicum.exception.ConditionsNotMetException;
import kz.yandex.practicum.exception.ValidationException;
...
import static kz.yandex.practicum.util.IdGenerator.getMaxIdOfUsers;
...
public class UserController {
    ...
    public ResponseEntity<User> updateUser(@Valid @RequestBody User user){
        ...
        User userToUpdate = Optional.ofNullable(users.get(user.getId()))
                .orElseThrow(() -> {
                    ...
                    return new ConditionsNotMetException("Не существует пользователя с id: " + user.getId());
                });
    ...
    private void validateUser(User user) {
        ...
        log.warn("Валидация логина {}", user.getEmail());
        if (userLogin.contains(SPACE) || user.getEmail().isEmpty()) {
